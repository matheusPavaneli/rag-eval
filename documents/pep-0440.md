...
- **Author:** Alyssa Coghlan <ncoghlan@gmail.com>, Donald Stufft <donald@stufft.io>
- **Status:** Final
...
Semantic versioning is a popular version identification scheme that is
...
ordering defined by the standard Version scheme. Whether or not
pre-releases are considered as candidate versions SHOULD be handled as
described in Handling of pre-releases.
...
Version scheme. Local version identifiers are NOT permitted in this
version specifier.
...
The specified version identifier must be in the standard format described in
Version scheme, but a trailing `.*` is permitted on public version
...
those of the Version matching operator, except that the sense of any
match is inverted.
...
more information on `file://` URLs on Windows see MSDN [4].
...
Ruby community's "pessimistic version constraint" operator [2] to allow
...
considered, please see the proof of concept for PEP 440 within pip [5].
...
* The normalized form for release candidates was updated from 'c' to 'rc'.
  ...
  preparing packages for publication on PyPI [8].
