...
this tension [#hugunin]; none have been really satisfactory.
...
of H \beta - r should perhaps be factored out; and,
...
numerical Python [#scipy-conf] [#pydata-conf].  At PyCon 2014, ~20%
...
footnote for discussion: [#github-details]), but, to the extent they
...
named [#matmul-other-langs]; here we discuss the various options.
...
\cdot, and has Perl/PHP baggage.  `$` is probably the
...
There was a long discussion [#associativity-discussions] about
whether `@` should be right- or left-associative (or even something
more exotic [#group-associativity]). Almost all Python operators are
...
users have an intuition that an expression like R S x proceeds
...
[#oil-industry-versus-right-associativity]), but at the least this
intuition is more common than for other operations like 2 \cdot 3 \cdot 4 which everyone reads as going from left-to-right.
...
right-associative nesting [#numpy-associativity-counts].  And anyway,
...
OS X) currently break the use of `multiprocessing` [#blas-fork].
...
[#preprocessor]) This suggestion seems based on the idea that
...
as in a well-known Python recipe:** (See: [#infix-hack]) Beautiful is
