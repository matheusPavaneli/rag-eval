...
- **Author:** Brett Cannon <brett@python.org>, Nathaniel J. Smith <njs@pobox.com>, Donald Stufft <donald@stufft.io>
- **Status:** Final
...
software for projects, distutils [#distutils] was the chosen
solution. As time went on, setuptools [#setuptools] gained popularity
...
its `setup()` function [#setup_args]. This solution has a number
of issues, such as:
...
All of this has led pip [#pip] to simply assume that setuptools is
...
`pyproject.toml` that is written in the TOML format [#toml].
...
Cargo package manager [#cargo] and in private email stated they have
...
Schema [#jsonschema] would match the data format:
...
positive arguments in favour of TOML) can be found at [#file_formats].
...
The JSON format [#json] was initially considered but quickly
...
The YAML format [#yaml] was designed to be a superset of JSON
[#json] while being easier to work with by hand. There are three main
issues with YAML.
...
PyYAML [#pyyaml] which is a large project of a few thousand lines of
...
configparser [#configparser] accepts was considered. Unfortunately
...
safe if parsed by `ast.literal_eval()` [#ast_literal_eval].
...
pypa.toml
  While it makes sense to reference the PyPA [#pypa], it is a
