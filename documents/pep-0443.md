...
reference implementation is available on hg.python.org [#ref-impl].
...
for the standard library [#pep-0008], and the best practices for
...
but created a number of edge cases with surprising results [#why-c3].
...
adaptation and method combining. PEAK-Rules [#peak-rules] is
...
In a 2005 article on Artima [#artima2005] Guido van Rossum presents
...
`generic` package available on PyPI [#pypi-generic], as well as David
Mertz's `gnosis.magic.multimethods` [#gnosis-multimethods].
...
PyPy's RPython offers `extendabletype` [#pairtype], a metaclass which
...
[#issue-5135] that proposed exposing `pkgutil.simplegeneric` as part
of the `functools` API, Guido van Rossum's article on multimethods
[#artima2005], and discussions with Raymond Hettinger on a general
