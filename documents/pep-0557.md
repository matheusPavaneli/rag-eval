...
- George Sakkis' recordType recipe [#], a mutable data type inspired
  by collections.namedtuple.

- Many example online recipes [#], packages [#], and questions [#].
  David Beazley used a form of data classes as the motivating example
  in a PyCon 2013 metaclass talk [#].
...
discussion, see [#].  Such `ClassVar` pseudo-fields are not
...
This discussion started on python-ideas [#] and was moved to a GitHub
repo [#] for further discussion.  As part of this discussion, we made
...
discussion [#] it was decided to keep consistency with
